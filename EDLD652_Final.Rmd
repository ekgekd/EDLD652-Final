...
#### The data used for this project are from https://www.kaggle.com/rtatman/blog-authorship-corpus, a text dataset of approximately 681,000 blogposts. For the purposes of this project, we will be working with one tenth of the full dataset. In addition to the text of each blog post, some information about each post (topic, date posted) and blogger (gender, age, astrological sign) is included.
...
#Amy: I added some line breaks here to keep the code from extending past 80 characters
exclu <- tibble(word = c("nbsp",
                         "urllink", 
                         "lot", 
                         "â", 
                         "im", 
...
tidy_words %>% 
  ...
  geom_col(fill = "cornflowerblue",
           alpha = .9) + #Amy: suggest a little bit of transparency
...
#Amy: One solution would be to use filter() - see code below
...
young <- data.frame(n_under20, under_20)
  ...
  geom_col(fill = "lightpink4",
           alpha = .9) + #Amy: suggest a little bit of transparency 
  theme_minimal() +
  scale_x_continuous(labels = scales::comma, expand = c(0,0)) + #Amy: suggest adding commas to x-axis
  theme(
    panel.grid.major.y = element_blank(),
    panel.grid.minor.x = element_blank(),
    panel.grid.major.x = element_line(color = "gray80")
  ...
  labs(
    ...
    caption = "Data abbreviated from https://www.kaggle.com/rtatman/blog-authorship-corpus" #Amy: Suggest adding caption
  )

#Amy: Potential solution using filter()   
tidy_words %>% 
  anti_join(stop_words) %>% 
  filter(agegroup == "Under 20") %>% 
  count(word, sort = TRUE) %>% 
  mutate(word = reorder(word, n)) %>% # make y-axis ordered by n
  slice(1:15) %>% # select only the first 15 rows
  ggplot(aes(n, word)) +
...
#Amy: Potential solution/alternate method using filter()  
tidy_words %>% 
  anti_join(stop_words) %>% 
  filter(agegroup == "20s") %>% 
  count(word, sort = TRUE) %>% 
...
<!-- Amy: Maybe it means industry unknown? I found this with the data on kaggle https://www.kaggle.com/rtatman/blog-authorship-corpus/discussion/114408 -->
...
topics <- tidy_words %>% 
  ...
  slice(20, 36, 14, 37, 5, 15) %>% #Amy: You could also use arrange(desc(topic_percent)) %>%  head(6) or slice(1:6) here.
  ggplot(aes(x = "", y = topic_percent, fill = topic)) +
  geom_col() +
  coord_polar(theta = "y") + 
  labs(title = "Proportion of blogs covering 5 most popular topics", #Amy: Should the title say 6 most popular topics or should code be updated to select top 5?
       x = "", 
       y = "") + 
  theme_minimal() 
...
rq3_plot <- rq3 %>% 
  ...
  ggplot(aes(x = date, 
             y = count,
             colour = agegroup)) +
  geom_point(alpha = .5, stroke = 0) + #Amy: Consider using stroke = 0 to remove outlines around points
  theme_bw() +
  scale_color_brewer(palette = "Blues") + #Amy: I really like this palette!
  ...
  theme(panel.grid.minor.y = element_blank(), #Amy: Could consider removing the minor axis grid lines
        panel.grid.minor.x = element_blank()) 

rq3_plot

#Amy: I really like how you fitted lines to the age groups and the blue palette you used. I found it a bit challenging to see patterns in the scatterplot points because there are so many and they're layered (i.e., they obscuring each other). I think you could consider faceting by age group. Other suggestions I have are adding stroke = 0 to geom_point to remove outline around points and considering removing the minor axis grid lines.
rq3 %>% 
  select(id, text, date, agegroup) %>% 
  group_by(id, text, date, agegroup) %>% 
  unnest_tokens(word, text) %>% 
  ...
  ggplot(aes(x = date, 
             y = count,
             colour = agegroup)) +
  geom_point(alpha = .5, stroke = 0) + #Amy: consider adding stroke = 0 so points don't have outlines
  theme_bw() +
  facet_wrap(~agegroup) + #Amy: consider facet wrap? (would need figure width adjusted)
  theme_bw() +
  ...
  labs(
    x = "Date Posted",
    y = "Number of Words",
    title = "Word Count by Blogger Age and Date",
    color = "Blogger Age" 
  ) +
  theme_minimal() +
  theme(panel.grid.minor.y = element_blank(), #Amy: Could consider removing the minor axis grid lines
