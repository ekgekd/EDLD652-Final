...
```{r rq1v1, warning= FALSE, echo = FALSE, message = FALSE}
...
tidy_words %>% 
  ...
  count(word, sort = TRUE) %>% 
  mutate(word = reorder(word, n)) %>% # make y-axis ordered by n
  slice(1:20) %>% # select only the first 20 rows
  ggplot(aes(n, word)) +
  geom_col(alpha = .7) +
  scale_x_continuous(expand = c(0,0),
                     labels = scales::comma) +
...
```{r q3v1, warning= FALSE, echo = FALSE, message = FALSE}
...
rq3_plot3 <- rq3 %>% 
  ...
  labs(
    ...
    y = "Number of Words",
    title = "Word Count by Blogger by Date",
    subtitle = "Bloggers in their 20s started blogging earlier than those younger than 20 or older than 30",
    color = "Blogger Age",
    caption = "Data abbreviated from https://www.kaggle.com/rtatman/blog-authorship-corpus") +
  facet_grid("agegroup")
